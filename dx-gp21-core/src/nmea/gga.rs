...
impl GgaData {
    pub fn is_valid(&self) -> bool { self.fix_quality != FixQuality::Invalid }
}
...
pub(crate) fn parse(_system: GnssSystem, f: &[&str]) -> Option<GgaData> {
    ...
    let geoid_sep: f32 = if f.len() > 10 { f[10].parse().unwrap_or(0.0) } else { 0.0 };
    Some(GgaData { time, lat, lon, fix_quality, sats_used, hdop, alt_msl, geoid_sep })
}
