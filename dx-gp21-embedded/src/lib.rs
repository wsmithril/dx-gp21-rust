...
use dx_gp21_core::nmea::{DhvData, GgaData, GsaData, GstData, GsvData, RmcData, VtgData, ZdaData};
...
impl<const N: usize> GnssStore for GnssState<N> {
    ...
    fn sats_in_view_count(&self) -> u8 {
        self.gga
            .map(|g| g.sats_used)
            .unwrap_or(self.satellites.len() as u8)
    }
}
...
impl<W: FnMut(&[u8]), const N: usize> EmbeddedSession<W, N> {
    ...
    pub fn state(&self) -> &GnssState<N> {
        &self.state
    }

    /// Mutable access to the current GPS state (rarely needed directly).
    pub fn state_mut(&mut self) -> &mut GnssState<N> {
        &mut self.state
    }
}
        ...
        R: embedded_io_async::Read,
    {
        ...
        loop {
            if let Ok(n) = reader.next_line(&mut buf).await
                && n > 0
                && let Some(s) = dx_gp21_core::feed_sentence(&mut self.state, &buf[..n])
            {
                return s;
            }
        }
